...
export async function POST(request: Request) {
  ...
  if (Array.isArray(body.records)) {
    ...
    await bulkUpsert(prepared, session.user.id);
    ...
    return NextResponse.json({ added: records.length, records });
  }
  ...
  const recordData = {
    ...
    tradedAt: toDate(body.tradedAt),
    note: body.note,
    stopLossPrice: body.stopLossPrice !== undefined ? Number(body.stopLossPrice) : null,
    takeProfitPrice:
      body.takeProfitPrice !== undefined ? Number(body.takeProfitPrice) : null,
    stopRule: body.stopRule || null,
    isLuZhu: Boolean(body.isLuZhu),
  };

  if (!recordData.symbol || !recordData.quantity || !recordData.price) {

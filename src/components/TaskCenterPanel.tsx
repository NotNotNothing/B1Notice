...
import { Loader2, Pause, Play, RefreshCw, RotateCcw, Square, Terminal } from 'lucide-react';
...
export function TaskCenterPanel() {
  ...
  const [selectedRun, setSelectedRun] = useState<TaskRunDetailView | null>(null);
  ...
  const loadOverview = async () => {
    const [nextDefinitions, nextRuns] = await Promise.all([
      fetch('/api/tasks').then((response) => readEnvelope<TaskDefinitionView[]>(response)),
      fetch('/api/tasks/runs?limit=20').then((response) => readEnvelope<TaskRunView[]>(response)),
    ]);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        await loadOverview();
        if (selectedRun?.id) {
          const detail = await fetch(`/api/tasks/runs/${selectedRun.id}`).then((response) =>
            readEnvelope<TaskRunDetailView>(response),
          );
        ...
        if (active) {
          toast.error(error instanceof Error ? error.message : '加载任务中心失败');
        }
      }
    };
  ...
  const handleDefinitionAction = async (
    ...
    try {
      ...
      const payload = await readEnvelope<TaskDefinitionView | TaskRunView>(response);
    ...
    <div className='space-y-6'>
              ...
              <CardHeader className='space-y-3'>
                <div className='flex items-start justify-between gap-3'>
                  <div className='space-y-2'>
                    ...
                    <CardDescription>{definition.schedule || '仅手动执行'}</CardDescription>
                  </div>
                  <Badge className={getStatusBadgeClassName(definition.latestRun?.status ?? definition.lastStatus)}>
                    {definition.isPaused
                      ? '已暂停'
                      : formatStatusLabel(definition.latestRun?.status ?? definition.lastStatus)}
                  </Badge>
                </div>
                <div className='flex flex-wrap gap-2'>
                  <Badge variant='outline'>{definition.category}</Badge>
                  <Badge variant='outline'>最大重试 {definition.maxRetries}</Badge>
                </div>
              </CardHeader>
              <CardContent className='space-y-4'>
                <div className='grid grid-cols-2 gap-3 text-sm'>
                  <div>
                    <p className='text-slate-500 dark:text-slate-300'>最近运行</p>
                    <p className='mt-1 font-medium text-slate-900 dark:text-white'>
                      {formatDateTime(definition.lastRunAt)}
                    </p>
                  </div>
                  <div>
                    <p className='text-slate-500 dark:text-slate-300'>最近成功</p>
                    <p className='mt-1 font-medium text-slate-900 dark:text-white'>
                ...
                <div className='rounded-2xl border border-slate-200 p-3 text-sm dark:border-slate-700'>
                  <p className='text-slate-500 dark:text-slate-300'>最新摘要</p>
                ...
                <div className='flex flex-wrap gap-2'>
                  <Button
                    onClick={() => void handleDefinitionAction(definition, 'run')}
      ...
      <div className='space-y-4'>
        <div className='flex items-center justify-between'>
          <h3 className='text-lg font-semibold text-slate-900 dark:text-white'>最近运行</h3>
          <p className='text-sm text-slate-500 dark:text-slate-300'>自动每 5 秒刷新一次</p>
        </div>
        ...
        <div className='space-y-3'>
            ...
            >
              <CardContent className='flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between'>
                <div className='space-y-3'>
                  ...
                  <div className='grid gap-2 text-sm text-slate-600 sm:grid-cols-2 lg:grid-cols-4 dark:text-slate-300'>
                  ...
                  <div className='rounded-2xl border border-slate-200 p-3 text-sm dark:border-slate-700'>
                    <p className='text-slate-900 dark:text-white'>{run.summary || '暂无摘要'}</p>
                    {run.errorMessage ? (
                      <p className='mt-2 text-rose-600 dark:text-rose-300'>{run.errorMessage}</p>
                    ) : null}
                  </div>
                </div>
                ...
                <div className='flex flex-wrap gap-2 lg:justify-end'>
                  ...
                  {run.status === 'RUNNING' || run.status === 'STOPPING' ? (
                    <Button
                      ...
                      disabled={actionKey === `stop-${run.id}` || run.status === 'STOPPING'}
                      className='rounded-xl'
                    >
          ...
          {!loading && runs.length === 0 ? (
            <Card className='rounded-3xl border border-dashed border-slate-300 bg-white/80 shadow-sm dark:border-slate-700 dark:bg-slate-900/70'>
              <CardContent className='flex min-h-[180px] items-center justify-center p-8 text-center text-slate-500 dark:text-slate-300'>
                暂无任务运行记录
              </CardContent>
            </Card>
          ) : null}
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className='mx-auto max-h-[85vh] w-[96vw] max-w-3xl overflow-y-auto rounded-3xl border-slate-200 bg-white/95 dark:border-slate-800 dark:bg-slate-900/95'>
          <DialogHeader>
          ...
          {selectedRun ? (
            <div className='space-y-4'>
              <div className='grid gap-4 sm:grid-cols-2'>
                <Card className='rounded-2xl border border-slate-200 shadow-none dark:border-slate-700'>
                  <CardContent className='space-y-2 p-4 text-sm'>
                    <div className='flex items-center justify-between'>
                      <span className='text-slate-500 dark:text-slate-300'>状态</span>
                      <Badge className={getStatusBadgeClassName(selectedRun.status)}>
                ...
                <Card className='rounded-2xl border border-slate-200 shadow-none dark:border-slate-700'>
                  <CardContent className='space-y-2 p-4 text-sm'>
                    <div>触发方式：{formatTriggerLabel(selectedRun.triggeredBy)}</div>
              ...
              {selectedRun.errorMessage ? (
                <Card className='rounded-2xl border border-rose-200 bg-rose-50/80 shadow-none dark:border-rose-900 dark:bg-rose-950/30'>
              ...
              <div className='space-y-3'>
                <h4 className='text-sm font-semibold text-slate-900 dark:text-white'>事件流</h4>
                <div className='space-y-3'>
                  {selectedRun.events.map((event) => (
                    <div
                      key={event.id}
                      className='rounded-2xl border border-slate-200 p-4 text-sm dark:border-slate-700'
                    >
                      <div className='flex flex-wrap items-center gap-2'>
                      ...
                      <p className='mt-2 text-slate-900 dark:text-white'>{event.message}</p>
                      {event.details ? (
                        <pre className='mt-3 overflow-x-auto rounded-xl bg-slate-100 p-3 text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-200'>
                          {event.details}
                        </pre>
                      ) : null}
                    </div>

...
export const AlertList = forwardRef<AlertListRef, AlertListProps>(
  ({ symbol }, ref) => {
    ...
    return (
      <div className="space-y-3">
        {monitors.map((monitor) => (
          <div
            key={monitor.id}
            className="bg-white dark:bg-gray-800 backdrop-blur-lg border border-gray-200 dark:border-gray-700 rounded-md p-3 sm:p-4 hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors duration-200 shadow-sm"
          >
            <div className="flex items-start sm:items-center justify-between gap-3">
              <div className="space-y-1 sm:space-y-2 min-w-0 flex-1">
                ...
                <div className="text-xs sm:text-sm text-gray-700 dark:text-gray-200 font-medium">
                  {monitor.type === "BBI_ABOVE_CONSECUTIVE" ||
                  monitor.type === "BBI_BELOW_CONSECUTIVE" ? (
                    <span>连续 {monitor.threshold} 天</span>
                  ) : (
                    <span>

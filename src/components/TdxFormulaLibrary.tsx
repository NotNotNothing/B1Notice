...
export function TdxFormulaLibrary({ onSelectFormula, currentFormula }: TdxFormulaLibraryProps) {
  ...
  const handleSave = async () => {
    try {
      ...
      const url = editingFormula ? `/api/tdx-formulas/${editingFormula.id}` : '/api/tdx-formulas';
          ...
          <div className='space-y-4 rounded-2xl border border-slate-200 p-4 dark:border-slate-700'>
            ...
            <div className='space-y-2'>
              ...
              <Textarea
                ...
                onChange={(event) =>
                  setFormData((current) => ({ ...current, formula: event.target.value }))
                }
            ...
            <div className='flex items-center justify-between'>
              ...
              <div className='flex gap-2'>
                ...
                <Button type='button' size='sm' onClick={handleSave} className='rounded-xl'>
        ...
        {loading ? (
          ...
          <div className='space-y-3'>
            {formulas.map((formula) => (
              <div
                key={formula.id}
                className='rounded-xl border border-slate-200 p-3 transition-all hover:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600'
              >
                <div className='mb-2 flex items-start justify-between'>
                  <div className='flex-1'>
                    <div className='flex items-center gap-2'>
                      <h4 className='font-medium text-slate-900 dark:text-white'>{formula.name}</h4>
                      {formula.isDefault && (
                        <Badge
                          variant='outline'
                ...
                <pre className='mb-3 overflow-x-auto rounded-lg bg-slate-100 p-2 text-xs font-mono text-slate-700 dark:bg-slate-800 dark:text-slate-300'>
                  {formula.formula}
                </pre>
                <div className='flex items-center justify-between'>
                  <div className='flex gap-2'>
                    <Button
                      ...
                      className='rounded-xl'
                    >
                      {currentFormula === formula.formula ? '当前使用' : '使用此公式'}
                    </Button>
                  </div>
                  <div className='flex gap-2'>
                    {!formula.isDefault && (
